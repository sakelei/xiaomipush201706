...
// 获取某个时间间隔内所有消息的状态。
func (m *MiPush) GetMessageStatusPeriod(ctx context.Context, beginTime, endTime int64) (*BatchStatusResultIncludeDescription, error) {
	params := m.assembleStatusPeriodParams(beginTime, endTime)
	bytes, err := m.doGet(ctx, m.host+MessagesStatusURL, params)
	if err != nil {
		return nil, err
	}
	var result BatchStatusResultIncludeDescription
	err = json.Unmarshal(bytes, &result)
	if err != nil {
		return nil, err
	}
...
func (m *MiPush) defaultForm(msg *Message) url.Values {
	...
	if len(msg.Subtitle) > 0 {
		form.Add("aps_proper_fields.title", msg.Subtitle)
	}
	...
	if len(msg.SoundUrl) > 0 {
		form.Add("extra.sound_url", msg.SoundUrl)
	}else {
		form.Add("extra.sound_url", "default")
	}
	if msg.NotifyID != 0 {
