...
type Message struct {
	...
	Title                 string            `json:"title,omitempty"`                   // 通知栏展示的通知的标题   (支持ios10.0以上)
	Subtitle              string            `json:"title,omitempty"`                   // 通知栏展示的通知的副标题 (支持ios10.0以上)
...
const (
	...
	MaxTimeToSearchMessage = time.Second * 3600 * 24 * 7 / 1e6
)
...
func (m *Message) SetTimeToSend(tts int64) *Message {
	if time.Since(time.Unix(0, tts*int64(time.Millisecond))) > MaxTimeToSend {
		...
		//m.TimeToSend = int64(MaxTimeToSend)
	} else {
		m.TimeToSend = tts
	}
	return m
...
func (m *Message) SetTimeToLive(ttl int64, timeToEx int64) *Message {
	...
	///tt1为s量级, 为保存时间长度
	if ttl*1e6 > int64(MaxTimeToLive) {
		m.TimeToLive = time.Now().Add(MaxTimeToLive).UnixNano()/1e6 - time.Now().UnixNano()/1e6
	} else {
		m.TimeToLive = ttl + timeToEx - time.Now().UnixNano()/1e6
	}
	return m
}
...
//-----------------------------------------------------------------------------------//
// 发送给IOS设备的Message对象
func NewIOSMessage(title, description string) *Message {
	return &Message{
		Payload:     "",
		Title:       title,
		Description: description,
